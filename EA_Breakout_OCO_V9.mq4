//+------------------------------------------------------------------+
//| EA_Breakout_OCO_V9.mq4                                        |
//| Breakout OCO Cycle Engine V9 (CSV-analysis Hardening)           |
//+------------------------------------------------------------------+
//
...
input string   EA_Name                  = "EA_Breakout_OCO_V9";
...
input string   CSV_FileName              = "EA_Breakout_OCO_V9_Behavior.csv";
...
input double   SellLotMult               = 0.75;  // [V9-01] reduce SELL risk (still worst side in V7.1a/V8)
...
int OnInit()
{
   ...
   LogEvent("EA_INIT", StringFormat("V9|Preset=%s|MaxLoss=$%.1f SL=$%.1f RiskPct=%.2f|HedgeMaxNeg=$%.1f LotRatio=%.2f ConfirmBars=%d Cooldown=%ds|DailyDD=%.1f%% WeeklyLim=$%.0f|AutoLotMode=%s|Session=%s NewsFilter=%s (%d times)",
