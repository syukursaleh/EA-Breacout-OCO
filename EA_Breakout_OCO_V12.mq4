//+------------------------------------------------------------------+
//| EA_Breakout_OCO_V12.mq4                                       |
//| Breakout OCO Cycle Engine V12 (Shallow Minus Block + Tighter Early Loss Cut) |
//+------------------------------------------------------------------+
//
...
//
// V8 CHANGELOG (from comparative analysis of EA_V6_log_XAUUSD.csv & EA_V7_log_XAUUSD.csv):
// Findings: time_loss_exit was 0% win-rate in both V6 and V7 samples (avg -15.5); soft_sl_momentum
// always closed at a large average loss (-25/-22) because it triggered too late; SELL OCO trades
// underperformed BUY OCO sharply (V6 avg -0.64 vs +5.27, V7 avg -11.53 vs +2.14); reversal-chaser
...
// [V8-02] (Priority 1) soft_sl_momentum now triggers earlier: SoftSL_USD default reduced 20.0->12.0
//         so the momentum-confirmed early exit fires before losses grow as large as in V6/V7 logs.
// [V8-03] (Priority 1) SELL entries require a stricter ADX confirmation (SellEntryExtraADX, default
//         +3.0 on top of the normal ADX threshold) since SELL OCO was the weakest setup in both logs.
// [V8-04] (Priority 2) Reversal-chaser is now only armed when the opposite-direction move is
//         confirmed by ADX (ChaserRequireADXConfirm / ChaserMinADXConfirm); otherwise the opposite
//         pending leg is simply deleted instead of arming a flip that historically always lost.
// [V8-05] (Priority 2) MaxSpreadPoints default tightened 45->38 (V6 log showed spread<35 trades were
//         far more profitable than spread 35-40 trades).
// [V8-06] (Priority 2) New SellLotMult input scales SELL-direction lot size down (default 0.85)
//         separately from CounterSideLotMult, since SELL underperformed BUY in both logs.
// [V8-07] (Priority 3) HedgeGivebackCapDollar default raised 1.50->2.50 to let profitable hedge
...
//
// V10 CHANGELOG (from analysis of EA_Breakout_OCO_V9_log_XAUUSD.csv, 116 closed live trades,
// net PnL -64.06, PF 0.88): the "main" position group alone lost -100.36 (hedge legs were net
// +36.30), and 83% of gross loss (-435.80 of -525.20) came from the time_loss_exit_confirmed /
// time_loss_exit_hardcap family. Root cause: with UseEarlyLossCut=false, there was NO real
// broker-side stop protecting a fresh position before TimeLossExitMinutes(8) elapsed or before
// the RSI-momentum confirmation (soft_sl_momentum required RSI extreme AND 2 opposite bars)
// fired, so losses regularly overshot the intended -12/-15.6 cap up to -30 whenever ATR was
// elevated (5-8) but still below the ATR_Volatility_Level1 lot-reduction threshold (8.0).
// SELL-side main trades were also still far weaker than BUY (-59.23 vs -1.38) despite the V9
// SellLotMult=0.75 reduction.
// [V10-01] (Priority 1) Enable UseEarlyLossCut=true (was false) so a real broker-side SL is
//          tightened to EarlyLossCut_MaxLoss/_WhileHedged while no meaningful peak profit has
//          formed yet, instead of relying only on tick-evaluated software exits that can be
//          skipped past by a fast adverse move.
// [V10-02] (Priority 1) Lower ATR_Volatility_Level1 8.0->6.0 and ATR_Volatility_Level2 10.0->8.0
//          so the 5-8 ATR band that produced the worst V9 overshoot losses now gets an
//          automatic lot-size reduction instead of trading full size.
// [V10-03] (Priority 1) Tighten TimeLossExit_HardMultiplier 1.3->1.15 so any position that does
//          slip past the RSI-confirmed cut is force-closed closer to TimeLossExitUSD instead of
//          being allowed up to 30% further loss.
// [V10-04] (Priority 2) SellLotMult reduced further 0.75->0.65; SELL main trades remained the
//          weaker side (-59.23 vs -1.38 for BUY) in the V9 live log.
...
//
// V12 CHANGELOG (from analysis of EA_Breakout_OCO_V11_log_XAUUSD.csv / _Behavior.csv, 36 closed
// "main" trades, net PnL -0.77): exit_reason breakdown was minus_block_force_close 16 trades
// +25.22 (avg +1.58, working well - left untouched), tiered_profit_trail 4 trades +55.02 (avg
// +13.76, very profitable but rarely reached because it only arms once peak >= TrailStart_Dollar
// =$8), failsafe_reconcile 3 trades -8.10, and broker_side_close(sl_or_tp_hit) 11 trades -72.61
// (avg -6.60) - the single largest loss bucket. Inspecting the 12 RECONCILED_BROKER_CLOSE rows in
// the Behavior CSV showed 10 of 12 closed at exactly -8.00/-8.01 (the EarlyLossCut_MaxLoss level),
// and most of those cycles had floated to a small peak profit (0.00/0.21/0.33/0.76/1.62/2.46/2.76/
// 2.91) that never reached MinusBlock_ArmPeakMoney=$3.0, so the existing Minus Block guard never
// armed and the position was allowed to round-trip all the way from a small profit down to the
// full -$8 early-loss-cut floor instead of being cut while still small/near breakeven.
// [V12-01] (Priority 1) NEW second/"shallow" Minus Block tier: UseMinusBlockShallow (default true)
//          arms once a cycle's peak floating profit reaches MinusBlockShallow_ArmPeakMoney
//          (default $1.0, i.e. below the original MinusBlock_ArmPeakMoney=$3.0 tier) and force-
//          closes the cycle if profit later retraces down to MinusBlockShallow_FloorMoney (default
//          -$1.0) while still well above the EarlyLossCut/emergency-loss floors. This closes the
//          exact gap identified above (peaks of $0.2-$2.9 that previously slid to -$8) without
//          touching the original Minus Block tier (MinusBlock_ArmPeakMoney/_FloorMoney), which is
//          left completely unchanged since it already averages +$1.58/trade.
// [V12-02] (Priority 1) EarlyLossCut_MaxLoss tightened 8.0->6.0 and EarlyLossCut_MaxLoss_WhileHedged
//          6.0->5.0: 10 of 12 broker-side closes in the V11 log hit exactly the old -8.0/-8.01
//          level, so the early-stage safety net was slightly looser than the current ATR/spread
//          conditions warrant; combined with V12-01 this should catch most of that loss bucket
//          earlier and smaller.
// [V12-03] (Priority 2) TrailStart_Dollar lowered 8.0->6.0 so more cycles reach the very profitable
//          tiered_profit_trail exit path (avg +$13.76 in the V11 log) instead of relying only on
//          the new shallow Minus Block tier for cycles that peak in the $6-$8 range.
// [V12-04] (Priority 3) NEW BuyLotMult input (default 1.00 = no behavior change) added to mirror the
//          existing SellLotMult, so a future analysis with a larger live sample can independently
//          scale BUY-side lot size down if the small BUY/SELL asymmetry seen in the V11 log
//          (BUY avg -0.44 vs SELL avg +0.42 per main trade) persists.
//+------------------------------------------------------------------+
#property strict
#property version   "12.0"
#property description "Breakout OCO V12 - Shallow Minus Block tier + tighter Early Loss Cut + BuyLotMult (from V11 live-trade CSV analysis)"

//============================== INPUTS ==============================
// --- General ---
input int      MagicNumber              = 9001110;
input string   EA_Name                  = "EA_Breakout_OCO_V12";
...
input double   TrailStart_Dollar         = 6.0;    // [V12-03] was 8.0: let more cycles reach the profitable tiered trail sooner
...
// --- Early Loss Cut ---
input bool     UseEarlyLossCut          = true;   // [V10-01] real broker-side early safety net (was false)
input double   EarlyLossCut_NoPeakAbove = 6.0;
input double   EarlyLossCut_MaxLoss     = 6.0;    // [V12-02] was 8.0: 10/12 V11 broker closes hit exactly the old -8.0 level
input double   EarlyLossCut_MaxLoss_WhileHedged = 5.0; // [V12-02] was 6.0
...
input int      TimeLossExitMinutes      = 8;       // [V9-03] reduce exposure time before loss-evaluation
...
input double   TimeLossExit_HardMultiplier = 1.15; // [V10-03] tighter hard-cap, reduce overshoot vs V9 (1.3)
...
input double   ATR_Volatility_Level1      = 6.0;  // [V10-02] catch the 5-8 ATR band that drove V9 overshoot losses
input double   ATR_Volatility_Level2      = 8.0;   // [V10-02]
...
// --- [V12-01] Shallow Minus Block: second, earlier-arming tier for cycles that never reach the
// original MinusBlock_ArmPeakMoney tier before reversing (V11 log: 10/12 broker-side closes had
// peaked at only $0.0-$2.9 and then round-tripped all the way to the -$8 EarlyLossCut floor) ---
input bool     UseMinusBlockShallow           = true;
input double   MinusBlockShallow_ArmPeakMoney = 1.00;  // arm once peak floating profit reaches this (below MinusBlock_ArmPeakMoney)
input double   MinusBlockShallow_FloorMoney   = -1.00; // force-close once profit retraces down to this (still well above EarlyLossCut)
...
input int      LossCooldownAfterLossSec  = 300;    // [V9-05] cooldown after each losing cycle to reduce overtrading
...
input double   SellLotMult               = 0.65;  // [V10-04] reduce SELL risk further (still worst side in V9 live log)
input double   BuyLotMult                = 1.00;  // [V12-04] mirrors SellLotMult for future BUY-side tuning; 1.00 = no change yet
...
input bool     ChaserRequireADXConfirm   = true;   // [V8-04] Require ADX confirmation before arming a reversal-chaser flip
input double   ChaserMinADXConfirm       = 22.0;   // [V8-04] Minimum ADX on the opposite direction to arm the chaser
...
input double   HedgeGivebackCapDollar   = 2.50;  // [V8-07] was 1.50: let profitable hedge trades run further before locking in
...
void OpenTradeLoggerIfNeeded()
{
   ...
   if(FileSize(g_tradeLogHandle) == 0)
      FileWrite(g_tradeLogHandle,
         ...
         "pnl","exit_reason","spread","atr","rsi","pyramid_level","balance","equity","comment");
...
void PlaceHedgePending()
{
   ...
   double hedgeWorstCaseLoss = hedgeLot * HedgeSL_Dollar * mppl;
   if(mppl > 0.0 && hedgeWorstCaseLoss > HedgeBlockMinusMaxLossDollar)
   {
      LogEvent("HEDGE_BLOCK_MINUS_CAP", StringFormat("hedgeLot=%.2f worstCase=%.2f cap=%.2f",
               hedgeLot, hedgeWorstCaseLoss, HedgeBlockMinusMaxLossDollar));
      return;
   }
   ...
   { pendingPrice = NormalizePrice(Ask + HedgePendingDistance);
     sl = NormalizePrice(pendingPrice - HedgeSL_Dollar); dirLabel = "BUY_STOP"; }
   else
   { pendingPrice = NormalizePrice(Bid - HedgePendingDistance);
     sl = NormalizePrice(pendingPrice + HedgeSL_Dollar); dirLabel = "SELL_STOP"; }
...
bool PlaceOCOOrders()
{
   ...
   double buyLot = NormalizeLot(lot * BuyLotMult); // [V12-04] mirrors SellLotMult below
   if(buyLot <= 0.0) buyLot = lot;
   if(!OrderSendWithRetry(Symbol(),OP_BUYSTOP,buyLot,buyP,SlippagePoints,buySL,buyTP,EA_Name+" BuyStop",MagicNumber,0,clrBlue))
   ...
   LogEvent("OCO_PLACED",StringFormat("buy=%d %.2f lot=%.2f sell=%d %.2f lot=%.2f dist=%.2f",g_buyStopTicket,buyP,buyLot,g_sellStopTicket,sellP,sellLot,dist));
   return true;
}
...
bool PlaceDirectionalOCO(int trend)
{
   ...
   if(trend==1)
   {
      double buyLot = NormalizeLot(lot * BuyLotMult); // [V12-04] mirrors SellLotMult in the bear branch below
      if(buyLot <= 0.0) buyLot = lot;
      double counterLot = NormalizeLot(buyLot * CounterSideLotMult);
      double buyP=NormalizePrice(Ask+dist), buySL=NormalizePrice(buyP-slDol), buyTP=CalculateTakeProfit(buyP,1);
      if(!OrderSendWithRetry(Symbol(),OP_BUYSTOP,buyLot,buyP,SlippagePoints,buySL,buyTP,EA_Name+" Bull BuyStop",MagicNumber,0,clrBlue))
      ...
      g_sellStopTicket=-1;
      LogEvent("DIR_OCO_PLACED",StringFormat("dir=BULL ticket=%d price=%.2f sl=%.2f tp=%.2f lot=%.2f dist=%.2f",g_buyStopTicket,buyP,buySL,buyTP,buyLot,dist));
   ...
   else
   {
      ...
      { if(!OrderSelect(i,SELECT_BY_POS,MODE_TRADES)) continue;
        if(OrderSymbol()==Symbol()&&OrderMagicNumber()==MagicNumber&&OrderType()==OP_SELLSTOP){g_sellStopTicket=OrderTicket();break;} }
      LogEvent("DIR_OCO_PLACED",StringFormat("dir=BEAR ticket=%d price=%.2f sl=%.2f tp=%.2f lot=%.2f dist=%.2f",g_sellStopTicket,sellP,sellSL,sellTP,sellLot,dist));
      if(UseDualSidedOCO && counterLot > 0.0)
      {
...
void HandleOCOTriggered()
{
   ...
   // [V8-04] Reversal-chaser flips always closed at a loss in the V6 log (reversal_chaser_flip avg -5.59, 0% win).
   // Only arm the chaser when the opposite-direction ADX is strong enough to justify a flip.
   bool chaserADXConfirmed = true;
   if(ChaserRequireADXConfirm)
   {
      double adxChaser = iADX(Symbol(), Trend_Timeframe, ADX_Period, PRICE_CLOSE, MODE_MAIN, 1);
      chaserADXConfirmed = (adxChaser >= ChaserMinADXConfirm);
   }
   if(P_UseReversalChaser && oppositeTicket > 0 && chaserCooldownOk && chaserUnderHourlyCap && chaserADXConfirmed)
   {
   ...
   else if(oppositeTicket > 0)
   {
      if(P_UseReversalChaser)
         LogEvent("REVERSAL_CHASER_SKIPPED_COOLDOWN", StringFormat(
                  "cooldownOk=%s flipsThisHour=%d/%d adxConfirmed=%s — deleting opposite leg instead",
                  chaserCooldownOk ? "true" : "false", g_chaserFlipsThisWindow, ChaserMaxFlipsPerHour,
                  chaserADXConfirmed ? "true" : "false"));
...
bool ExitDecisionEngine()
{
   ...
   // rows) before reversing. Arms earlier (MinusBlockShallow_ArmPeakMoney) and force-closes once
   ...
   // [V8-01] Time Loss Exit redesign (was: blind timeout+loss cutoff, 0% win-rate in V6/V7 logs).
...
void RecordCycleResult(double cyclePnL)
{
   ...
   if(cyclePnL < -1.0)
   {
      g_consecutiveLosses++;
      g_dailyLosses++;
      if(LossCooldownAfterLossSec > 0)
      {
         datetime lossPauseUntil = TimeCurrent() + LossCooldownAfterLossSec;
         if(lossPauseUntil > g_lossPauseUntil) g_lossPauseUntil = lossPauseUntil;
         LogEvent("LOSS_COOLDOWN_AFTER_LOSS", StringFormat("cyclePnL=%.2f pauseUntil=%s", cyclePnL,
...
void ApplyPreset()
{
   ...
   if(PresetMode == 1)
   {
      ...
      P_HedgeLotRatio=0.75; P_MaxDailyDrawdownPct=2.0; P_ADX_Threshold=28.0;
   ...
   else if(PresetMode == 2)
   {
      ...
      P_HedgeLotRatio=1.00; P_MaxDailyDrawdownPct=3.0; P_ADX_Threshold=25.0;
   ...
   else if(PresetMode == 3)
   {
      ...
      P_HedgeLotRatio=1.25; P_MaxDailyDrawdownPct=4.0; P_ADX_Threshold=22.0;
...
int OnInit()
{
   ...
   LogEvent("EA_INIT", StringFormat("V12|Preset=%s|MaxLoss=$%.1f SL=$%.1f RiskPct=%.2f|HedgeMaxNeg=$%.1f LotRatio=%.2f ConfirmBars=%d Cooldown=%ds|DailyDD=%.1f%% WeeklyLim=$%.0f|AutoLotMode=%s|Session=%s NewsFilter=%s (%d times)",
